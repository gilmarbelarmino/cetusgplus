    ...
    if ($method === 'POST') {
        ...
        if ($action === 'add_volunteer') {
            ...
            $stmt = $pdo->prepare("INSERT INTO volunteers (id, name, cpf, avatar_url, gender, email, phone, unit_id, sector_id, volunteering_sector, location, profession, hourly_rate, start_date, work_area, 
                ...
                total_hours, points, status, company_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?, ?,?,'Ativo', ?)");
            ...
            $stmt->execute([
                ...
                $total_hours, floor($total_hours), $compId
            ]);
        ...
        if ($action === 'edit_volunteer') {
            ...
            $stmt->execute([
                ...
                floatval($data['hours_may']??0), floatval($data['hours_jun']??0), floatval($data['hours_jul']??0), floatval($data['hours_aug']??0),
                floatval($data['hours_sep']??0), floatval($data['hours_oct']??0), floatval($data['hours_nov']??0), floatval($data['hours_dec']??0),
                $total_hours, floor($total_hours), $current_id, $vid, $compId
            ]);
        ...
        if ($action === 'inativar') {
            ...
            if ($vol) {
                $pdo->prepare("INSERT INTO volunteer_history (volunteer_id, start_date, end_date, total_hours, edited_by, edited_at, company_id) VALUES (?, ?, CURDATE(), ?, ?, NOW(), ?)")
                    ->execute([$vid, $vol['start_date'], $vol['total_hours'], $current_id, $compId]);
        ...
        if ($action === 'reativar') {
            ...
            $pdo->prepare("UPDATE volunteers SET status = 'Ativo', end_date = NULL, start_date = CURDATE(), total_hours = 0, points = 0,
                ...
                ->execute([$current_id, $vid, $compId]);
            
            $pdo->prepare("INSERT INTO volunteer_history (volunteer_id, start_date, total_hours, edited_by, edited_at, company_id) VALUES (?, CURDATE(), 0, ?, NOW(), ?)")
                ->execute([$vid, $current_id, $compId]);
